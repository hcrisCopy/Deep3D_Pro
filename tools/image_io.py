...
def read_image(file_path, mod='normal', loadsize=0, rgb=False):
    """Read an image with optional Windows-friendly path handling."""
...
def write_image(file_path, img, use_thread=False):
    """Write an image while preserving Windows support for non-ASCII paths."""
    suffix = os.path.splitext(file_path)[-1]

    def subfun(path, image):
        if system_type == 'Linux':
    ...
    if use_thread:
        thread = Thread(target=subfun, args=(file_path, img), daemon=True)
        thread.start()
    else:
...
def resize(img, size, interpolation=cv2.INTER_CUBIC):
    """Resize while preserving aspect ratio using the shorter side as reference."""
    h, w = img.shape[:2]
    if np.min((w, h)) == size:
        return img
    if w >= h:
