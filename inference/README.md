...
#### B. Fast Mode：降低模型输入分辨率

脚本：

```bash
python inference/benchmark_speed_fast_mode.py \
  --gpu_id 0 \
  --model ../data/pretrained/deep3d_v1.0_1280x720_cuda.pt \
  --data_root ../data/test_set/speed_test \
  --out_root ../data/test_on_speed \
...
```text
../data/test_on_speed/YYYYMMDD/HHMMSS_fast_960x540_to_1920x1080/
```
...
Fast Mode 的核心是降低模型真正处理的画面尺寸，把最耗时的推理计算从高分辨率转移到低分辨率上完成，再把预测出的右视图恢复到目标输出尺寸。这样做不会改变输入输出流程的整体形态，最终仍然可以得到 `1920x1080` 的左右拼接视频，但中间推理阶段的计算量会明显下降。

默认 `960x540` 相比 `1280x720`，有效计算面积约为 `56.25%`，通常属于速度和画质比较均衡的配置。`640x360` 相比 `1280x720`，有效计算面积约为 `25%`，加速空间更大，更适合端侧算力紧张或目标 FPS 压力较大的场景。

这条路线的代价也比较明确：模型在低分辨率上看到的纹理、边缘和细小视差都会减少，放大回 1080p 后可能出现细节偏软、边界不够稳定、局部视差不够精确的问题。因此汇报时建议把 FPS 提升和可视化样张放在一起展示，说明它是“速度优先”的取舍方案，而不是完全无损优化。

适用结论：

- 最有效的加速路线，属于“以画质换速度”。
- 适合端侧算力受限、目标 FPS 优先的场景。
- 汇报时建议同时展示 `visualizations/`，说明速度收益和画质代价。
...
这类优化通常不会带来 Fast Mode 那样大幅度的速度提升，因为它没有减少画面分辨率和主干计算规模。但它的优势是风险低，输出质量理论上应与 baseline 保持一致，适合作为第一组无损加速实验。对汇报来说，可以把它定位为“工程部署侧的轻量优化”，用于说明在不改画质目标的情况下还能获得多少额外收益。
...
- 属于“无画质损失”的轻量推理优化。
- 收益通常小于 Fast Mode，但风险低、易部署。
- 推荐作为 baseline 后的第一组无损优化对照。

---

#### D. Cached Grid：缓存固定坐标
...
- 适合和 JIT 优化、Fast Mode 做组合路线的后续探索。
...
| `--alpha` | `5` | 时序远帧偏移 |
| `--model_size` | 从模型名解析 | 模型输入尺寸，Fast Mode 默认 `960 540` |
| `--output_size` | baseline/JIT/cached-grid 自动策略；Fast Mode 默认 `1920 1080` | 最终视频和可视化输出尺寸 |
...
→ Fast Mode steady_model_fps / visual quality
```
...
| 4 | `benchmark_speed_fast_mode.py --model_size 960 540` | 评估默认 fast mode 的速度/质量平衡 |
| 5 | `benchmark_speed_fast_mode.py --model_size 640 360` | 评估更激进端侧方案 |
...
- Fast Mode：最强加速，代价是上采样带来的细节和视差精度损失。
